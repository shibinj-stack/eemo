/* ============================================================
   ...
   ============================================================ */
const API_URL = 'https://eemo11.onrender.com/api/predict'; // Use your actual backend URL here
const EMOTIONS = {
...
const state = {
  ...
  pauses: 0, backspaces: 0, startTime: null, lastKeyTime: null, totalKeys: 0,
  PAUSE_THRESHOLD: 800,
};

// ── Server health check ───────────────────────────────────────
window.addEventListener('load', () => {
  fetch('https://eemo11.onrender.com/health')
    .then(r => r.json())
    .then(() => {
      document.getElementById('serverStatus').textContent = 'ML Engine Ready';
      const dot = document.getElementById('statusDot');
      if (dot) { dot.style.background = '#4ecdc4'; dot.style.boxShadow = '0 0 8px #4ecdc4'; }
    })
    .catch(() => {
      document.getElementById('serverStatus').textContent = 'Server Offline — run app.py';
      const dot = document.getElementById('statusDot');
      if (dot) dot.style.background = '#f4504a';
    });
});
...
textarea.addEventListener('keydown', (e) => {
  ...
  if (state.keyEvents.length > 0) {
    const last = state.keyEvents[state.keyEvents.length - 1];
    if (last.downTime) state.flightTimes.push(now - last.downTime);
  }
...
function buildPayload() {
  ...
  return {
    ...
    pauseRate:      parseFloat((state.pauses / Math.max(elapsed * 60, 1)).toFixed(4)),
